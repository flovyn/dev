...
| Phase | Status | Notes |
|-------|--------|-------|
| Phase 1 | ✅ Complete | Added `WorkflowOutcome<T>` type in `sdk-rust/sdk/src/workflow/outcome.rs` |
...
Created `sdk-rust/sdk/src/workflow/outcome.rs` with:
...
Changes to `sdk-rust/sdk/src/error.rs`:
...
| File | Changes |
|------|---------|
| `sdk-rust/sdk/src/workflow/outcome.rs` | NEW - `WorkflowOutcome<T>` type with Ready/Suspended/DeterminismViolation |
| `sdk-rust/sdk/src/workflow/mod.rs` | Added `pub(crate) mod outcome` |
| `sdk-rust/sdk/src/workflow/future.rs` | Added `WorkflowFuturePoll` trait, `poll_outcome()` to all futures |
| `sdk-rust/sdk/src/error.rs` | Added `#[non_exhaustive]`, `#[doc(hidden)]`, error category docs |
