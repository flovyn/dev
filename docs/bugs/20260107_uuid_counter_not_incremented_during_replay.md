...
**Component:** `sdk-rust/sdk/src/workflow/context_impl.rs`
**Severity:** Critical
...
In `sdk-rust/sdk/src/workflow/context_impl.rs`, the `uuid_counter` is always initialized to 0 when a new `WorkflowContextImpl` is created (line 178):
...
- `sdk-rust/sdk/src/workflow/context_impl.rs` - Added `uuid_counter` increment in replay path (line 696)
- `sdk-rust/sdk/src/workflow/context_impl.rs` - Added regression test `test_multiple_child_workflows_after_replay_get_unique_uuids`

## Why E2E Tests Didn't Catch This

The existing e2e tests in `sdk-rust/sdk/tests/e2e/child_workflow_tests.rs` only test:
